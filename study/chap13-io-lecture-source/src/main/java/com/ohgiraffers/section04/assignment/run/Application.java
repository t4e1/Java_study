...
public class Application {
    ...
    /* 설명. 사용자로부터 회원 번호를 제외한 정보를 입력받아(회원가입용 정보) Member타입으로 반환하는 메소드(파싱 및 가공처리) */
    private static Member signUp() {
        Member newInfo = null;                  // 반환형을 먼저 정의한다.
        ...
        int age = sc.nextInt();
        System.out.print("입력할 취미 개수를 입력하세요(숫자로 1이상): ");
        int length = sc.nextInt();
        sc.nextLine();                          // Scanner 버퍼에 남아있는 개행 제거
        ...
        /* 필기.
         *  회원으로부터 회원가입을 위한 정보를 입력받아 Member타입 객체 하나로 가공처리할 때 방법이 두가지가 있다.
         *  1. 생성자 방식 (장: 한줄 코딩 가능, 단: 따로 생성자 추가 필요)
         *  2. setter 방식 (장: 초기화 할 갯수 수정 용이, 단: 코딩 줄 수가 늘어날 수 있음)
        * */
