...
public class ExceptionHandlerServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        /* 설명.
         *  request는 사실상 하나의 요정부터 응답까지 유지되는 임시 저장 공간이기도 하다.
         *  이 공간에 값을 담기 위해서는 setAttribute()라는 메소드를 써서 담을 수 있고
         *  값을 다시 추출하고자 할 때는 getAttribute()라는 메소드를 쓸 수 있따.(키와 밸류 형태로)
        * */

        /* 설명. request attribute에는 에러와 관련 된 정보가 들어있다. 학인 불가 */
        Enumeration<String> attrNames = req.getAttributeNames();
        while (attrNames.hasMoreElements()) {
            System.out.println(attrNames.nextElement());
        }

        /* 설명. request의 attribute에 에러 관련된 것들을 활용하여 에러 전용 페이지를 만들어 응답한다. */
