...
public class OrderService {
    ...
    * */
    /* 설명. 서비스 메소드 내부에서 모든 DML 관려 작업에 예외 없이 잘 동작하면 commit, 예외가 발생하면 rollback이 적용된다. */
    /* 설명.
     *  전파행위 옵션
     ...
     *                  오염된 값 읽기 문제는 해결되지만 재현 불가능한 값 읽기 및 허상읽기는 여전히 발생할 수 있다. (다른 row 수정 및 추가는 막을 수 없다)
     *  REPEATABLE_READ : 트랜젝션이 어떤 필드를 여러 번 읽어도 동일한 값을 읽도록 보장한다.
     *                   트랜젝션이 지속되는 동안에는 다른 트랜젝션이 해당 필드를 변경할 수 없다.
     *                   오염된 값 읽기, 재현 불가능한 값 읽기는 해결되지만 허상읽기는 여전히 발생할 수 있다. (다른 row 추가는 막을 수 없다)
     ...
     *  READ_COMMITTED ~ SERIALIZABLE 까지의 격리레벨을 설정할 수 있다 -> 최하위인 ReAD_COMMITED로 디폴트를 잡는게 일반적
     * 설명.
    ...
    public void registNewOrder(OrderDTO orderInfo) {
        ...
        /* 설명. 1. 주문한 메뉴 코드를 추출(DTO에서) */
//        List<Integer> menuCode = orderInfo.getOrderMenus()
//                .stream().map(OrderMenuDTO::getMenuCode)
...
//        System.out.println("map : " + map.get("menuCodes"));
...
//                        .map(dto -> {
//                            return new OrderMenu(dto.getMenuCode(), dto.getOrderAmount());  // DTO를 엔티티로 옮겨 담는 작업
//                        }).collect(Collectors.toList())
//        );
        List<OrderMenu> oMenus = new ArrayList<>();
    ...
    private int calcTotalOrderPrice(List<OrderMenuDTO> orderMenus, List<Menu> menus) {
        ...
        for (int i = 0; i < orderMenuSize; i++) {
            ...
            totalOrderPrice += orderMenu.getOrderAmount() * menu.getMenuPrice();
        }
