...
public class Application3_1 {
    ...
    public static int solution(String input) throws IOException {
        ...
        int[][] time = new int[N][2];               // 2(시작시간, 종료시간) 만큼의 크기를 가지는 N개의 행렬을 담은 2차원 배열
        StringTokenizer str;                        // 띄워쓰기가 있기 때문에 토크나이저 사용
        for (int i = 0; i < N; i++) {
        ...
        * */
        Arrays.sort(time, (o1, o2) -> { // Comparator, Comparable 차이 정리하기

            /* 설명. 종료시간이 같은 회의에 대해서는 */
            if (o1[1] == o2[1]) {       // 종료 시간이 같으면

                /* 설명. 시작 시간이 늦은 순서로(내림차순) 정렬 기준을 잡는다.(회의 시간이 짧아지는 순서로 쌓기 위함) */
                return o2[0] - o1[0];   // 시작 시간이 늦은 순(가장 짧은 회의 시간 순) 으로 내림차순
            }

            /* 설명. 종료시간이 같지 않은 경우에는 종료 시간이 이른 회의부터 정렬되도록 기준을 잡는다.(종료시간 오름차순) */
            return o1[1] - o2[1];
        });
