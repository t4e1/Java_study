...
public class Application2 {
    ...
    /* 설명. 필요한 배추 흰 지렁이 개체 수 */
    static int count;

    /* 설명. x와 y좌표를 인지하는 내부 static 클래스(위에서도 static을 사용했기 때문에 전부 static을 사용해야 활용 가능) */
    static class Node {
        int x;
        int y;
    ...
    public static int solution(String input) throws IOException {
        ...
        count = 0;
        for (int i = 0; i < N; i++) {       // 바깥쪽 for문 (행의 개념 = y좌표)
            for (int j = 0; j < M; j++) {   // 안쪽 for문 (열의 개념 = x좌표)
