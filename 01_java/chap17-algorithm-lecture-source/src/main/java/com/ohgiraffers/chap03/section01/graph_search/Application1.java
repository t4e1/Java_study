...
public class Application1 {
    ...
    /* 설명. 문제에서 요구하는 입력 값을 받아 감연된 컴퓨터의 갯수(int/Integer)를 반환하는 메소드 작성 */
    public static Integer solution(String input) throws IOException {
        ...
        map = new int[node + 1][node + 1];          // 번호와 index 위치를 일치시키기 위해서 크기를 1씩 증가시킴(0번 인덱스는 사용 안함)

        /* 설명. 방문 배열 작성(방문 배열을 작성하는 이유는 방문했던 노드를 다시 stack에 담지 않거나 다시 재귀 함수 호출을 하지 않기 위함 */
        visit = new boolean[node + 1];              // 방문 배열도 node 번호와 index를 일치시키기 위해서 1 증가

        /* 설명. 인접리스트(map)로 그래프에 대한 정보 작성 */
        /* 입력받은 간선의 정보를 map에다가 표시함*/
