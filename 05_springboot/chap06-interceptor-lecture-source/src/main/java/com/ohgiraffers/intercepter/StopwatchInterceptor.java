...
public class StopwatchInterceptor implements HandlerInterceptor {
    ...
    /* 설명. 필터와 달리 인터셉터는 bean을 활용할 수 있다.(생성자 주입 활용) */
    ...
    /* 설명. boolean형에 따라 핸들러 메소드가 실행 될 수도 있고 안 될 수도 있도록 할 수 잇으며 핸들러 메소드 이전 전처리 목적이다. */
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {

        /* 설명. 핸들러 인터셉터는 bean을 활용할 수 있다. (@Service 계층의 객체도 bean이다.) */
//        MENUSERVICE.method();   // 핸들러 메소드가 실행되기 전이지만 뒷단에 있는 Service의 메소드를 호출할 수 있다.
