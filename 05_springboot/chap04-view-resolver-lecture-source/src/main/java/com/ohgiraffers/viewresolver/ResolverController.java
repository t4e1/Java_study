...
//        return "redirect:/?message1=ttt";
    ...
    /* 설명. 단순 forward 시에는 String 반환과 Model을 활용하는 코드와 ModelAndView를 사용한 코드가 차이가 없다. */
    @GetMapping("modelandview")
    public ModelAndView modelAndViewTest(ModelAndView mv) {
        mv.addObject("message2", "ModelAndView를 이용한 forward");
        mv.setViewName("result");       // 기존에 String 으로 반환한 값을 여기에 지정
    ...
    public ModelAndView modelAndViewRedirect(ModelAndView mv) {
        ...
        /* 설명. ModelAndView를 통한 리다이렉트 시에는 addObject 한 것이 parameter(?가 있는 쿼리스트링 형태)로 넘어간다. */
        mv.addObject("message2", "ModelAndView를 이용한 redirect");
        mv.setViewName("redirect:/");   // "redirect:/?message2=ModelAndView를+이용한+redirect" 로 넘어간다.
