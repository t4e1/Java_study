...
//        System.out.println("After Advice 동작");
    ...
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        ...
        /* 설명. AfterReturing Advice를 활용한 반환값 가공도 가능하다.  */
        if (result != null && result instanceof List) {
            ((List<MemberDTO>) result).add(new MemberDTO(3L, "반환 값 가공"));
        }
    }

    /* 설명. 4. AfterThrowing Advice */       // 타겟이 예외를 발생시켰을 때 동작하는 메소드
     ...
     *  AroundAdvice 의 조인포인트 매개변수는 ProceedingJoinPoint로 고정되어 있다.
     *  JoinPoint의 하위 인터페이스로 원본 조인포인트의 진행 시점을 제어할 수 있다.
     *  조인포인트를 진행하는 호출을 잊는 경우가 자주 발생하기 떄문에 주의해야하며
    ...
    * */
    ...
    public Object logAround(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        /* 설명. 실행된 타겟 메소드 반환(다른 어드바이스도 활용할 수 있도록 반환한다.) */
        return result;
    }
}
