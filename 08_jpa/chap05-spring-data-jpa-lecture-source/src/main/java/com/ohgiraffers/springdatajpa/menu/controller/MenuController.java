...
//    Logger logger = LoggerFactory.getLogger(getClass());
    ...
    @GetMapping("/{menuCode}")  // path variable 방식 : 경로 상에 있는걸 바로 변수로 받아내는 방식
    public String findMenuByCode(@PathVariable int menuCode, Model model) {
//        logger.info("menuCode: {}", menuCode);
        log.info("before menuCode: {}", menuCode);
...
//        log.info("여러 페이지 중 현재 인덱스 : {}", menuList.getNumber());
    ...
    /* 설명. /menu/regist.html 이 열리자마자 js 코드를 통해 /menu/category 비동기 요청이 오게 된다. */
    @GetMapping(value = "/category", produces = "application/json; charset=UTF-8")

    /* 설명. 메소드에 @ResponseBody가 붙은 메소드의 반환형은 ViewResolver가 해석하지 않는다. */
    /* 설명.
     *  @ResponseBody가 붙었을 때 기존과 다른 핸들러 메소드 차이점
     *  1. 핸들러 메소드의 반환형이 어떤 것이라도 상관 없다. (-> 모두 json  문자열 형태로 요청이 들어온 곳으로 반환된다.)
     *  2. 한글이 포함된 데이터는 produces 속성에 'application/json'이라는 MIME 타입과 'charset=UTF-8'의 인코딩 타입을 붙여준다.
    * */
    @ResponseBody
