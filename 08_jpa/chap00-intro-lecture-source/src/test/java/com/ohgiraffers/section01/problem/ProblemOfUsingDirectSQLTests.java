...
public class ProblemOfUsingDirectSQLTests {
     ...
     *  2. SQL에 의존하여 개발해야 함
     *  3. 패러다임 불일치(상속, 연관관계, 객체 그래프 탐색, 방향성 등의 java 개념을 JDBC로 구현하기 힘듬)
     *   방향성 : 자바는 A->B 또는 B->A 등의 방향성을 가지지만, DB는 양방향성을 가진다.
     *  4. 동일성 보장 문제 (SQL은 조회할 때마다 새로운 주소를 가진 객체가 된다)
    * */

    /* 설명. 1. 데이터 변환, SQL 작성, JDBC API 코드 등의 중복 작성(개발 시간 증가, 유지보수성 저하) */
    @DisplayName("직접 SQL을 작성해서 메뉴를 조회할 때 발생하는 문제 확인")
    @Test
    ...
    /* 설명.  3-1. 상속 문제
     *  객체 지향 언어의 상속 개념과 유사한 것이 데이터베이스의 서브타입엔티티이다.(서브타입을 별도 클래스로 나뉘었을 때)
     *  슈퍼타입의 모든 속성을 서브타입이 공유하지 못하여 물리적으로 다른 테이블로 분리가 된 형태이다.
     *  (설계에 따라서는 하나의 테이블로 속성이 추가되곧 한다.)
    ...
    * */
    ...
    void testEquals() throws SQLException {
        ...
        // then
        Assertions.assertNotEquals(menu1, menu2);   // 같은 쿼리를 조회해온 두 menu 가 다른 객체임을 알 수 있다.
         ...
         *      -> 매번 새로운 객체를 만들면 메모리 낭비 문제도 있고, 상태값이 다를 수도 있다.
         *      -> JPA는 같은 쿼리를 쓴 객체들을 같은 객체로 처리한다.
        * */
