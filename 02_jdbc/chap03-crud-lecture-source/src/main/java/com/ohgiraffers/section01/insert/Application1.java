...
public class Application1 {
    ...
    public static void main(String[] args) {
        ...
        /* 설명.
         *  DML(insert, update delete) 작업 시에는 반환 결과가 int값이 된다. (ResultSet을 사용하지 않는다.)
         *  그리고 기존의 조회일 때는 excuteQuery()를 사용했지만, DML 작업 시에는 executeUpdate()를 사용한다.
        * */
        int result = 0;         // CRUD의 결과로는 ResultSet이 아니라 int가 온다.
        ...
        try {
            ...
            * */
            result = pstmt.executeUpdate();     // 자동 커밋 상태에서는 excuteUpdate하면 자동 커밋된다.

            if (result > 0) {
                System.out.println("insert 결과 : " + result + ", 수동 커밋하기 !");
                con.commit();   // 제대로 insert가 되었으면 commit해라 ( feat. 수동 커밋 변환 - 커넥션이 만들어지는 시점에서 변환 )
            } else {
                con.rollback();
            }
